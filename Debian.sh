...
# wget -qO- "https://source.zhijie.online/AutoDeploy/main/Debian.sh" | sudo bash
...
function GetSystemInformation() {
    ...
    function CheckMachineEnvironment() {
        ...
        function CheckHypervisorEnvironment() {
            ...
            if [ "$?" -eq "1" ]; then
                ...
                for source_list_task in "${!source_list[@]}"; do
                    sed -i "s/deb.debian.org/mirrors.ustc.edu.cn/g;s|security.debian.org/debian-security|mirrors.ustc.edu.cn/debian-security|g" "${source_list[$source_list_task]}"
                done && apt update && apt install virt-what -qy
                which "virt-what" > "/dev/null" 2>&1
                if [ "$?" -eq "1" ]; then
    ...
    function GenerateHostname() {
        ...
        if [ $(echo "${NEW_HOSTNAME}" | wc -l) -ne 1 ] || [ "${RESET_HOSTNAME}" == "true" ]; then
            NEW_HOSTNAME="Debian-$(date '+%Y%m%d%H%M%S')"
        fi
    }
    ...
    function GetLSBCodename() {
        ...
        if [ "$(awk -v NUM1=$LSBVersion_CURRENT -v NUM2=$LSBVersion 'BEGIN{print (NUM1 > NUM2) ? 1 : 0}')" -eq "1" ]; then
