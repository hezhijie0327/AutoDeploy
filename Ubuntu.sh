...
function InstallCustomPackages() {
    ...
    function InstallXanModKernel() {
        ...
        echo "deb [arch=${OSArchitecture} signed-by=/usr/share/keyrings/xanmod-archive-keyring.gpg] https://deb.xanmod.org releases main non-free" > "/etc/apt/sources.list.d/xanmod.list"
        apt update && for apt_list_task in "${!apt_list[@]}"; do
            apt-cache show ${apt_list[$apt_list_task]} && if [ "$?" -eq "0" ]; then
                apt install -qy ${apt_list[$apt_list_task]}
            fi
