...
function ConfigurePackages() {
    ...
    function ConfigureUfw() {
        ...
        if [ "$?" -eq "0" ] && [ -f "/etc/default/ufw" ]; then
            ...
            if [ "${container_environment}" != "docker" ] && [ "${container_environment}" != "wsl2" ]; then
                ufw reload && ufw insert allow 123/udp && ufw limit 22/tcp && ufw allow 323/udp && ufw allow 51820/udp && ufw limit 9022/tcp && ufw allow 9090/tcp && ufw enable && ufw status verbose
            else
