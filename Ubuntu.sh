...
function ConfigurePackages() {
    ...
    function ConfigureUfw() {
        ...
        if [ "$?" -eq "0" ] && [ -f "/etc/default/ufw" ]; then
            ...
            ufw reload && ufw reset && ufw allow 123/udp && ufw allow 161/udp && ufw limit 22/tcp && ufw allow 323/udp && ufw allow 3493/tcp && ufw allow 51820/udp && ufw limit 9022/tcp && ufw allow 9090/tcp && ufw allow from 10.172.0.0/16 && ufw enable && ufw status verbose
        fi
    }
