...
function InstallDependencyPackages() {
    ...
    if [ -d "$(brew --repo)/Library/Taps/homebrew" ]; then
        ...
        done && brew update && brew install bash curl git git-flow git-lfs jq knot mas mercurial nano neofetch p7zip rar unzip vim wget youtube-dl zip zsh && compaudit | xargs chmod g-w,o-w && brew cleanup
    fi
}
# Upgrade Packages
