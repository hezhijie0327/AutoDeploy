...
function ConfigurePackages() {
    function ConfigureCrontab() {
        crontab_list=(
            ...
            "0 0 * * 7 export PATH=\"/opt/homebrew/bin:/opt/homebrew/sbin:/usr/local/sbin:\$PATH\" && brew update && brew upgrade --greedy \$(brew outdated --greedy | xargs) && brew cleanup && mas upgrade && softwareupdate -ai"
        )
...
function UpgradePackages() {
    brew update && brew upgrade --greedy $(brew outdated --greedy | xargs) && brew cleanup && mas upgrade && softwareupdate -ai
}
