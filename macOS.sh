...
function InstallDependencyPackages() {
    ...
    if [ -d "$(brew --repo)/Library/Taps/homebrew" ]; then
        ...
        done && brew update && brew install bash curl ffmpeg git git-flow git-lfs gnupg jq knot mas mercurial nano neofetch p7zip rar unzip vim wget youtube-dl zip zsh && brew cleanup
    fi
}
# Upgrade Packages
