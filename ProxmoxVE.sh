    ...
    function ConfigureOpenSSH() {
        ...
        if [ "$?" -eq "0" ]; then
            ...
            if [ -d "/etc/pve/priv" ]; then
                rm -rf "/etc/pve/priv/authorized_keys" "/etc/pve/priv/known_hosts" && touch "/etc/pve/priv/authorized_keys" && touch "/etc/pve/priv/known_hosts" && chmod 600 "/etc/pve/priv/authorized_keys" && chmod 644 "/etc/pve/priv/known_hosts"
            fi
