    ...
    function ConfigureGrub() {
        ...
        if [ "$?" -eq "0" ]; then
            if [ -f "/etc/kernel/cmdline" ]; then
                ...
                fi && cat "/etc/kernel/cmdline.bak" | sed "s/boot=zfs/boot=zfs quiet security=apparmor mitigations=auto,nosmt${PATCH_INTEL_82599_SFP}${DISABLE_DISPLAY}${ENABLE_IOMMU}/g" > "/etc/kernel/cmdline" && proxmox-boot-tool refresh
            fi
        fi
    }
